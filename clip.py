...
extn_regex=r'\.\w+$'
start_validation=re.fullmatch(time_regex,start)
end_validation=re.fullmatch(time_regex,end)
# -- verify valid time formats
if not start_validation:
    print("Bad format for start time. XX:XX:XX or XX:XX:XX.XXX")
    exit(99)
if not end_validation:
    print("Bad format for end time. XX:XX:XX or XX:XX:XX.XXX")
    exit(99)
# -- figure out if the source file exists
...
target_filename=os.path.basename(filename)[:amt_to_truncate]+'.clipped.'+datetime.datetime.now().strftime("%Y%m%d.%H%M%S")+'.mp4'
